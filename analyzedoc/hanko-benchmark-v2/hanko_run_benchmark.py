#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Hanko Occlusion — benchmark runner (resume-safe).
Sibling of the Legibility Frontier runner: MODELS/RATES/api/upload/retry logic are
verbatim from that harness (behavioral parity). One variable: stamp overlap L0-L5.

Usage:
  python3 hanko_run_benchmark.py --dry-run
  python3 hanko_run_benchmark.py --models pilot --yes            # 3 models x full grid
  python3 hanko_run_benchmark.py --models all --yes              # 27 variants (matsu)
  python3 hanko_run_benchmark.py --models all --instances A_N,B_N --yes   # ume (name target only)
  python3 hanko_run_benchmark.py --models ume --instances A_N --t1-reps 3 --yes  # free-tier repro

The extraction prompt never mentions the stamp or how to treat occluded text —
that choice is the measurand (design sheet section 4)."""
...
INSTANCES = ["A_N","B_N","A_M","B_M"]              # opacity x target
T1_OUT_CHARS = 380
UPLOAD_TTL_H = 24
...
RATES = {  # (page_rate, output_rate/1000 chars) — billing.json 2026-08-11
 ...
 "anthropic/claude-fable-5":(1909,1500),"anthropic/claude-sonnet-5":(382,300),"anthropic/claude-opus-4-8":(955,750),
...
T1_PROMPT = (
 "Extract the following 14 fields from this Japanese invoice, exactly as printed in the "
 ...
 "- issuer_name: the issuing company name\n"
 ...
 "- stamp_text: the text of the red square seal, if present"
)
T1_EXAMPLE = {"doc_title":"","total":"","invoice_no":"","counterparty_name":"","issuer_name":"",
              "issue_date":"","due_date":"","subtotal":"","tax":"","bank_name":"","bank_branch":"",
              "account_number":"","issuer_tel":"","stamp_text":""}
...
def enumerate_jobs(models, reps, mat_dir, instances=INSTANCES):
    jobs = []
    for m in models:
        for inst in instances:
            for li, pct in enumerate(OVERLAPS):
                for rep in range(reps):
                    jobs.append(dict(key=f"h|{m}|{inst}|L{li}|r{rep}", task="t1", model=m,
                                     doc=inst, L=li, pct=pct, rep=rep,
                                     path=f"{mat_dir}/hanko_{inst}_L{li}.png",
...
def run_one(job, file_ids, key, out_path, poll_max, wait_s, job_retries):
    t0 = time.time()
    rec = {k: job[k] for k in ("key","task","model","doc","L","pct","rep")}
...
def main():
    ...
    ap.add_argument("--materials", default="materials_hanko")
    ap.add_argument("--out", default="hanko_results.jsonl")
    ...
    insts = [i.strip() for i in a.instances.split(",")]
    bad = [i for i in insts if i not in INSTANCES]
    if bad: sys.exit(f"unknown instances: {bad}")

    jobs = enumerate_jobs(models, a.t1_reps, a.materials, insts)
    missing = [j["path"] for j in jobs if not os.path.exists(j["path"])]
    if missing: sys.exit(f"missing materials ({len(missing)}), e.g. {missing[0]} - run hanko_gen_materials.py first")
    ...
    print(f"estimated pending cost: {cr:,.0f} credits (approx ${cr*1e-4:,.2f} list)")
    if a.dry_run: return
    ...
    file_ids = upload_all(sorted({j["path"] for j in pending}), key, cache_path="hanko_uploads.json")
    ...
    print("next: python3 hanko_score_results.py && python3 hanko_report.py")
