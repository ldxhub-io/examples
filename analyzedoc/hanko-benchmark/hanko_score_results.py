#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Hanko Occlusion — deterministic scorer.
Field classification (correct/near/blank/fabricated) is verbatim from the Legibility
Frontier scorer for cross-benchmark comparability. Adds stamp_text as a reference-only
class (correct/blank/other; excluded from frontiers)."""
...
def main():
    res_path = sys.argv[1] if len(sys.argv) > 1 else "hanko_results.jsonl"
    gt = json.load(open("hanko_ground_truth.json"))
    fields = gt["fields"]; seal = gt["stamp_text"]
    last = {}
    for line in open(res_path, encoding="utf-8"):
        try: rec = json.loads(line)
        except Exception: continue
        last[rec["key"]] = rec
    out = open("hanko_scores.jsonl", "w", encoding="utf-8")
    n_ok = n_fail = n_badjson = 0
    for rec in last.values():
        base = {k: rec[k] for k in ("key","model","doc","L","pct","rep")}
        ...
        n_ok += 1
        for field, g in fields.items():
            got = data.get(field)
            r = dict(base); r.update(field=field, tier=g["tier"],
                                     cls=classify(field, got, g), got=str(got)[:120] if got is not None else "")
            out.write(json.dumps(r, ensure_ascii=False)+"\n")
        got = data.get("stamp_text")
        s = base_norm(got) if got is not None else ""
        cls = "blank" if s == "" else ("correct" if s == base_norm(seal) else "other")
        r = dict(base); r.update(field="stamp_text", tier="stamp", cls=cls, got=str(got)[:60] if got is not None else "")
        out.write(json.dumps(r, ensure_ascii=False)+"\n")
    out.close()
    print(f"scored: parsed_ok={n_ok} job_failed={n_fail} bad_json={n_badjson} -> hanko_scores.jsonl")
